...
def getToken():
    ...
    if os.path.isfile(st.BOT_TOKEN_FILENAME):
        ...
        print("Нет доступа к токену. Проверьте или создайте в папке проекта файл 'token.txt', поместите туда токен от botFather  и запустите скрипт заново")
        sys.exit()
    return token
...
def game(callBackData):
    ...
    if not buttonNumber == 9:  
        ...
        if charList[buttonNumber] == st.SYMBOL_UNDEF:  
            ...
            if is_win(charList, st.SYMBOL_X):  
                ...
                if countUndefinedCells(charList) != 0:  
                    ...
                    while (isCycleContinue):
                        ...
                        if charList[rand] == st.SYMBOL_UNDEF:
                            ...
                            if is_win(charList, st.SYMBOL_O):
